...
void __no_inline_not_in_flash_func(flash_range_erase2)(uint32_t flash_offs, size_t count) {
...
void __no_inline_not_in_flash_func(flash_range_program2)(uint32_t flash_offs, const uint8_t *data, size_t count) {
#ifdef PICO_FLASH_SIZE_BYTES
    hard_assert(flash_offs + count <= PICO_FLASH_SIZE_BYTES);
#endif
...
void flash_get_unique_id2(uint8_t *id_out) {
#if PICO_NO_FLASH
